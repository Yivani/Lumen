...
class LumenIcarusEditor(ctk.CTk):
    ...
    def add_input_field(self, label_text, key_id, value, section, icon_color=None):
        ...
        entry = ctk.CTkEntry(row, font=("Consolas", 15), fg_color=COL_BG_INPUT, border_width=0, corner_radius=0, text_color=COL_TEXT_LIGHT, justify="right")
    ...
    def calc_xp_from_lvl(self):
        try:
            ...
            self.xp_entry_ref.delete(0, "end"); self.xp_entry_ref.insert(0, str(new_xp))
    ...
    def calc_lvl_from_xp(self):
        try:
            ...
            if lvl < 1: lvl = 1
            self.lvl_entry_ref.delete(0, "end"); self.lvl_entry_ref.insert(0, str(lvl))
    ...
    def load_data(self):
        ...
        if os.path.exists(p_path):
            try:
                ...
                currency_config = {"Refund": {"label": "RESPEC POINTS", "color": C_REF}, "Credits": {"label": "REN", "color": C_REN}, "Exotic1": {"label": "EXOTICS", "color": C_EXO}, "Exotic_Red": {"label": "STABILIZED", "color": C_RED}, "Biomass": {"label": "LEGENDARY BIOMASS", "color": C_BIO}, "licence": {"label": "LEGENDARY LICENSE", "color": C_LIC}}
                ...
                for item in meta:
                    ...
                    if key in currency_config:
                        ...
                        if conf["label"] not in added_labels: self.add_input_field(conf["label"], key, item.get("Count", 0), "PROFILE", icon_color=conf["color"]); added_labels.append(conf["label"])
                for key, conf in currency_config.items():
                    if conf["label"] not in added_labels: self.add_input_field(conf["label"], key, 0, "PROFILE_NEW", icon_color=conf["color"]); added_labels.append(conf["label"])
        ...
        if os.path.exists(c_path):
            try:
                ...
                raw_char_data = None
                if isinstance(container, list) and len(container) > 0: self.char_key_or_index = 0; raw_char_data = container[0]
                elif isinstance(container, dict) and len(container) > 0: first_key = next(iter(container)); self.char_key_or_index = first_key; raw_char_data = container[first_key]
                if isinstance(raw_char_data, list) and len(raw_char_data) > 0: raw_char_data = raw_char_data[0]; self.char_is_nested_list = True
                else: self.char_is_nested_list = False
                final_char_obj = {}
                if isinstance(raw_char_data, str):
                    try: final_char_obj = json.loads(raw_char_data); self.char_is_encoded_string = True
                    except: pass
                elif isinstance(raw_char_data, dict): final_char_obj = raw_char_data; self.char_is_encoded_string = False
                if final_char_obj:
    ...
    def save_data(self):
        ...
        try:
            ...
            if self.char_container is not None and os.path.exists(c_path):
                ...
                if self.char_is_nested_list and isinstance(raw_data, list): item_to_edit = raw_data[0]
                else: item_to_edit = raw_data
                char_obj = {}
                if self.char_is_encoded_string and isinstance(item_to_edit, str): char_obj = json.loads(item_to_edit)
                elif isinstance(item_to_edit, dict): char_obj = item_to_edit
                for key_full, entry in list(self.entries.items()):
                    section, r_key = key_full.split("|")
                    if section == "CHAR":
                ...
                if self.char_is_encoded_string: final_element = json.dumps(char_obj)
                if self.char_is_nested_list: self.char_container[self.char_key_or_index][0] = final_element
                else: self.char_container[self.char_key_or_index] = final_element

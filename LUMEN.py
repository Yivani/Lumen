...
class LumenIcarusEditor(ctk.CTk):
    def __init__(self):
        ...
        if self.profiles_map:
            first_id = next(iter(self.profiles_map.keys()))
            self.profile_dropdown.set(first_id)
            self.after(100, lambda: self.change_profile(first_id))
        else:
    ...
    def cleanup_old_backups(self):
        ...
        try:
            ...
            if len(backup_folders) >= max_backups:
                to_delete = len(backup_folders) - max_backups + 1
                deleted_count = 0
